...
LOG_FILE = "logs.txt"
ADMIN_ACTIONS_LOG_FILE = "admin_actions.txt"  # Лог действий админа

# --- GOOGLE SHEETS ---
SPREADSHEET_ID = "15vlEZ0Q6OmQh51DsA9B_fgiLwed12ekroz1aeWsgXVI"
WORKSHEET_NAME = "Логи клиентов"

# --- СТАТУСЫ И ИХ ЭМОДЗИ ДЛЯ ТЕМ ---
STATUS_EMOJI = {
    "Новый": "🟢",
    "Уточняем нишу": "🟡",
    "Уточняем оборот": "🟡",
...
# Эмодзи для тем в зависимости от статуса
TOPIC_EMOJI = {
    "Новый": "🆕",
    "Уточняем нишу": "📝",
    "Уточняем оборот": "💰",
...
BUTTONS = {
    "niche": [
        ...
        {"text": "👤 Связаться с человеком", "value": "human"},
        {"text": "◀️ Назад", "value": "back"}
    ],
    "turnover": [
...
# Вопросы с индикатором этапа
def get_question_with_stage(question_num, total=4):
    stages = ["нишу", "оборот", "проблему", "источник"]
    return f"📊 Этап {question_num}/{total}: {QUESTIONS[question_num]}\n\n{stages[question_num-1]}"
...
user_stage = {}
user_answers = {}
user_question_start_time = {}  # Время начала каждого вопроса
admin_actions_log = []
client_codes = {}
QR_TEXT = "https://t.me/add_production"
...
def log_admin_action(admin_id, action, target_id=None, details=""):
    """Логирует действия админа в файл"""
    ...
    # Записываем в файл
    try:
        with open(ADMIN_ACTIONS_LOG_FILE, "a", encoding="utf-8") as f:
            f.write(log_entry + "\n")
    except Exception as e:
        logger.error(f"Ошибка записи лога действий: {e}")
    
    logger.info(log_entry)

def get_time_response(question_num, user_id):
    """Возвращает время ответа на вопрос в секундах"""
    if user_id in user_question_start_time and question_num in user_question_start_time[user_id]:
        elapsed = time.time() - user_question_start_time[user_id][question_num]
        return round(elapsed, 1)
    return 0

def save_answer_time(user_id, question_num):
    """Сохраняет время ответа в заметки"""
    if user_id in user_question_start_time and question_num in user_question_start_time[user_id]:
        elapsed = time.time() - user_question_start_time[user_id][question_num]
        add_note_to_client(user_id, f"⏱️ Время ответа на вопрос {question_num}: {round(elapsed, 1)} сек")
        return round(elapsed, 1)
    return 0
        ...
        for i in range(len(all_data)-1, 0, -1):
            if len(all_data[i]) > 1 and all_data[i][1] == str(user_id):
                return {
                    ...
                    "response_time": all_data[i][8] if len(all_data[i]) > 8 else ""
                }
        return None
    except Exception as e:
...
def update_client_response_time(user_id, response_time):
    if not worksheet: return False
    try:
        all_data = worksheet.get_all_values()
        if len(all_data) < 2: return False
        ...
        for i in range(len(all_data)-1, 0, -1):
            if len(all_data[i]) > 1 and all_data[i][1] == str(user_id):
                ...
                break
        if last_row_index:
            # Получаем текущее время ответа
            current = ""
            if len(all_data[last_row_index-1]) >= 9:
                current = all_data[last_row_index-1][8]
            new_time = f"{current} {response_time}".strip() if current else response_time
            worksheet.update_cell(last_row_index, 9, new_time)
            logger.info(f"✅ Время ответа {user_id} → {response_time}")
            return True
        return False
    except Exception as e:
...
def log_to_sheets(user_id, username, first_name, message_text, status="Новый", source="", response_time=""):
    if not worksheet:
        logger.warning(f"⚠️ worksheet не инициализирован, пропускаем запись для {user_id}")
        return
    try:
        if is_first_message(user_id):
            timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            row = [timestamp, str(user_id), f"@{username}" if username else "нет", first_name or "нет", message_text, status, "", source, response_time]
            ...
            if response_time:
                update_client_response_time(user_id, response_time)
    except Exception as e:
        logger.error(f"❌ Ошибка записи для {user_id}: {e}")
...
def init_google_sheets():
    ...
    try:
        ...
        try:
            ...
            new_headers = ["Timestamp", "User ID", "Ник клиента", "Имя", "Сообщение", "Статус", "Заметки", "Источник", "Время ответа"]
            ...
            worksheet = spreadsheet.add_worksheet(title=WORKSHEET_NAME, rows=1000, cols=9)
            worksheet.append_row(["Timestamp", "User ID", "Ник клиента", "Имя", "Сообщение", "Статус", "Заметки", "Источник", "Время ответа"])
            format_worksheet(worksheet)
...
# === СОЗДАНИЕ ТЕМЫ (С ЭМОДЗИ ПО СТАТУСУ) ===
async def get_or_create_topic(context, user_id, username, first_name):
    ...
    topic_emoji = TOPIC_EMOJI.get(current_status, "🆕")
    topic_name = f"{topic_emoji} {client_code} | {first_name}"
    ...
    try:
        ...
        # Пытаемся получить аватарку клиента
        try:
...
async def send_simple_message_to_topic(context, topic_id, client_text, bot_response, status):
    ...
    # Обновляем название темы с новым статусом
    try:
        new_topic_name = f"{TOPIC_EMOJI.get(status, '🆕')} {client_code} | {client_code.split('|')[1] if '|' in client_code else client_code}"
        await context.bot.edit_forum_topic(chat_id=GROUP_ID, message_thread_id=topic_id, name=new_topic_name[:128])
    except Exception as e:
        logger.warning(f"Не удалось обновить название темы: {e}")
...
# === КОНТЕКСТНЫЙ ОТВЕТ ===
def get_contextual_question(question_num, answers):
    """Возвращает вопрос с учётом контекста (что уже знаем о клиенте)"""
    if question_num == 2 and answers.get("niche"):
        return f"Понял! Вы работаете в нише {answers['niche']}. А какой сейчас примерный ежемесячный оборот?"
    elif question_num == 3 and answers.get("turnover"):
        return f"Спасибо! Оборот {answers['turnover']} — хороший масштаб. А в чём сейчас основная сложность с продажами?"
    elif question_num == 4 and answers.get("problem"):
        return f"Благодарю! Понимаю вашу проблему с {answers['problem']}. И последний вопрос — откуда узнали о нас?"
    return get_question_with_stage(question_num, 4)
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    logger.info(f"🔘 Кнопка нажата: {data} от {user_id}")
    ...
    if data == "restart_confirm":
        ...
        if user_id in user_question_start_time:
            del user_question_start_time[user_id]
        
        update_client_status(user_id, "Новый")
        ...
        user_answers[user_id] = {"niche": "", "turnover": "", "problem": "", "source": ""}
        log_to_sheets(user_id, user.username or "нет", user.first_name or "нет", "/start (перезапуск)", status="Новый")
        
        greeting = get_greeting()
        welcome_msg = f"{greeting}, {user.first_name}! 👋\n\nЯ виртуальный помощник ADD production.\n\nДавайте начнём новый диалог.\n\n📊 Этап 1/4"
        
        await query.edit_message_text(welcome_msg)
        
        client_code = generate_client_code(user_id)
        await context.bot.send_message(chat_id=GROUP_ID, message_thread_id=topic_id, text=f"👤 **Новый диалог!**\n🔑 Код: {client_code}\n📝 Начинаем заново...")
        
        # Сохраняем время начала вопроса
        if user_id not in user_question_start_time:
            user_question_start_time[user_id] = {}
        user_question_start_time[user_id][1] = time.time()
        
        await context.bot.send_message(
            chat_id=user_id,
            text=get_question_with_stage(1, 4),
            reply_markup=get_buttons_for_question("niche")
        )
        log_to_sheets(user_id, user.username or "нет", user.first_name or "нет", "Вопрос: ниша (перезапуск)", status="Уточняем нишу")
        await schedule_reminder(context, user_id)
        return
    
    if data == "restart_cancel":
        await query.edit_message_text("❌ Отменено. Если передумаете, просто напишите /start снова.")
        return
    ...
    if stage <= 4 and stage in user_question_start_time.get(user_id, {}):
        elapsed = save_answer_time(user_id, stage)
        log_to_sheets(user_id, user.username or "нет", user.first_name or "нет", f"Время ответа на вопрос {stage}: {elapsed} сек", response_time=f"{elapsed} сек")
    ...
    if answer == "back":
        if stage > 1:
            ...
            # Сохраняем время начала предыдущего вопроса
            if user_id not in user_question_start_time:
                user_question_start_time[user_id] = {}
            user_question_start_time[user_id][prev_stage] = time.time()
            
            await query.edit_message_text(
                text=f"◀️ Вы вернулись к предыдущему вопросу\n\n{get_contextual_question(prev_stage, answers)}",
                reply_markup=get_buttons_for_question(prev_question)
            )
            update_client_status(user_id, ["Уточняем нишу", "Уточняем оборот", "Уточняем проблему", "Уточняем источник"][prev_stage - 1])
        else:
            await query.edit_message_text("❌ Это первый вопрос, назад нельзя")
        return
    ...
    if answer == "human":
        ...
        if topic_id:
            await send_simple_message_to_topic(
                context, topic_id,
                "Клиент запросил связь с человеком", 
                "Передан руководителю",
                "Передан руководителю"
            )
        
        log_admin_action(ADMIN_ID, "клиент запросил связь", user_id, "нажал кнопку 'Связаться с человеком'")
        return
    ...
    if question_type == "niche":
        ...
        # Сохраняем время начала следующего вопроса
        if user_id not in user_question_start_time:
            user_question_start_time[user_id] = {}
        user_question_start_time[user_id][2] = time.time()
        
        await query.edit_message_text(f"✅ Принято: {answer}\n\n{get_contextual_question(2, answers)}")
        await context.bot.send_message(
            chat_id=user_id,
            text=get_contextual_question(2, answers),
        ...
        if user_id not in user_question_start_time:
            user_question_start_time[user_id] = {}
        user_question_start_time[user_id][3] = time.time()
        
        await query.edit_message_text(f"✅ Принято: {answer}\n\n{get_contextual_question(3, answers)}")
        await context.bot.send_message(
            chat_id=user_id,
            text=get_contextual_question(3, answers),
        ...
        if user_id not in user_question_start_time:
            user_question_start_time[user_id] = {}
        user_question_start_time[user_id][4] = time.time()
        
        await query.edit_message_text(f"✅ Принято: {answer}\n\n{get_contextual_question(4, answers)}")
        await context.bot.send_message(
            chat_id=user_id,
            text=get_contextual_question(4, answers),
        ...
        times = []
        for q in range(1, 5):
            if q in user_question_start_time.get(user_id, {}):
                elapsed = time.time() - user_question_start_time[user_id][q]
                times.append(f"{q}: {round(elapsed, 1)}с")
        response_time_str = ", ".join(times)
        
        log_to_sheets(user_id, user.username or "нет", user.first_name or "нет", f"Источник: {answer}", status="Передан руководителю", source=answer, response_time=response_time_str)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Если клиент уже был передан руководителю или отказался
    if client_info and client_info.get("status") in ["Передан руководителю", "Отказ"]:
        keyboard = [
            [
                InlineKeyboardButton("✅ Да, начать новый диалог", callback_data="restart_confirm"),
                InlineKeyboardButton("❌ Нет, позже", callback_data="restart_cancel")
            ]
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)
        
        await update.message.reply_text(
            "🔄 Ваш предыдущий запрос уже обработан.\n\n"
            "Если вы хотите начать новый диалог, нажмите 'Да'.\n"
    ...
    # Инициализируем время начала вопроса
    if user.id not in user_question_start_time:
        user_question_start_time[user.id] = {}
    user_question_start_time[user.id][1] = time.time()
    
    log_to_sheets(user.id, user.username or "нет", user.first_name or "нет", "/start", status="Новый")
    ...
    await update.message.reply_text(
        get_question_with_stage(1, 4),
        reply_markup=get_buttons_for_question("niche")
    )
    log_to_sheets(user.id, user.username or "нет", user.first_name or "нет", "Вопрос: ниша", status="Уточняем нишу")
    await schedule_reminder(context, user.id)
    
    log_admin_action(ADMIN_ID, "новый диалог", user.id, f"клиент {user.first_name} начал диалог")
...
async def handle_client_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if any(word in msg_lower for word in ["позвоните", "свяжитесь", "напишите", "человек", "менеджер"]):
        ...
        log_admin_action(ADMIN_ID, "клиент запросил связь", user_id, "написал запрос на связь")
        return
    ...
    if stage == 1:
        ...
        # Сохраняем время ответа
        if 1 in user_question_start_time.get(user_id, {}):
            elapsed = save_answer_time(user_id, 1)
            log_to_sheets(user_id, user.username or "нет", user.first_name or "нет", f"Время ответа на вопрос 1: {elapsed} сек", response_time=f"{elapsed} сек")
        ...
        if 2 in user_question_start_time.get(user_id, {}):
            elapsed = save_answer_time(user_id, 2)
            log_to_sheets(user_id, user.username or "нет", user.first_name or "нет", f"Время ответа на вопрос 2: {elapsed} сек", response_time=f"{elapsed} сек")
        ...
        if 3 in user_question_start_time.get(user_id, {}):
            elapsed = save_answer_time(user_id, 3)
            log_to_sheets(user_id, user.username or "нет", user.first_name or "нет", f"Время ответа на вопрос 3: {elapsed} сек", response_time=f"{elapsed} сек")
        ...
        if 4 in user_question_start_time.get(user_id, {}):
            elapsed = save_answer_time(user_id, 4)
        ...
        for q in range(1, 5):
            if q in user_question_start_time.get(user_id, {}):
                if q == 4:
                    elapsed_final = time.time() - user_question_start_time[user_id][q]
                    times.append(f"{q}: {round(elapsed_final, 1)}с")
                else:
                    # Времена уже сохранены в заметках
                    pass
        response_time_str = ", ".join(times)
        ...
        add_note_to_client(user_id, summary)
        log_to_sheets(user_id, user.username or "нет", user.first_name or "нет", f"Источник: {answers['source']}", status="Передан руководителю", source=answers['source'], response_time=response_time_str)
...
async def handle_thanks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    client_info = get_client_info(user_id)
    if client_info and client_info.get("status") == "Передан руководителю":
        if any(word in message.text.lower() for word in ["спасибо", "благодарю", "thanks"]):
            await message.reply_text("🌟 Всегда рады помочь! Хорошего дня! 🌟")

# === КОМАНДА ДЛЯ ПРОСМОТРА ЛОГА ДЕЙСТВИЙ ===
@catch_errors
async def admin_actions_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if os.path.exists(ADMIN_ACTIONS_LOG_FILE):
            with open(ADMIN_ACTIONS_LOG_FILE, "rb") as f:
                await update.message.reply_document(f, filename="admin_actions.txt")
        else:
            await update.message.reply_text("Логов действий админа пока нет")
    except Exception as e:
        await update.message.reply_text(f"Ошибка: {e}")

# === АНАЛИТИКА ПО ВРЕМЕНИ ОТВЕТА ===
@catch_errors
async def response_time_stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id != ADMIN_ID:
        return
    ...
    try:
        ...
        for row in all_data[1:]:
            if len(row) >= 9 and row[8]:
                # Парсим время ответа (формат: "1: 5.2с, 2: 3.1с" и т.д.)
                time_str = row[8]
                import re
                numbers = re.findall(r'(\d+\.?\d*)с', time_str)
                for num in numbers:
                    times.append(float(num))
        
        if not times:
            await update.message.reply_text("📭 Нет данных о времени ответов")
            return
        ...
        text = f"📊 **СТАТИСТИКА ВРЕМЕНИ ОТВЕТА**\n\n"
        text += f"📝 Всего замеров: {len(times)}\n"
        text += f"⏱️ Среднее время: {round(avg_time, 1)} сек\n"
        text += f"⚡ Минимальное: {round(min_time, 1)} сек\n"
        text += f"🐢 Максимальное: {round(max_time, 1)} сек\n"
        
        await update.message.reply_text(text)
        
    except Exception as e:
        logger.error(f"❌ Ошибка статистики времени: {e}")
...
async def handle_admin_reply_in_topic(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    log_admin_action(ADMIN_ID, "ответил клиенту", client_id, message.text[:50])
...
async def note_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if message.reply_to_message:
        ...
        if add_note_to_client(client_id, note_text):
            ...
            log_admin_action(ADMIN_ID, "добавил заметку", client_id, note_text)
        else:
...
async def ban_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        log_admin_action(ADMIN_ID, "заблокировал клиента", user_id, reason)
    except ValueError:
...
async def unban_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if user_id in blacklist:
            ...
            log_admin_action(ADMIN_ID, "разблокировал клиента", user_id)
        else:
...
async def info_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        text += f"⏱️ Время ответов: {info.get('response_time', '—')}\n"
        if answers:
            text += f"\n📌 **Ответы клиента:**\n"
...
async def broadcast_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    log_admin_action(ADMIN_ID, "инициировал рассылку", details=f"текст: {broadcast_text[:50]}...")
...
async def broadcast_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "broadcast_confirm":
        ...
        log_admin_action(ADMIN_ID, "завершил рассылку", details=f"успешно: {success}, ошибок: {failed}")
...
def main():
    ...
    app.add_handler(CommandHandler("admin_actions", admin_actions_command))
    app.add_handler(CommandHandler("response_time", response_time_stats))
    ...
    app.add_handler(CallbackQueryHandler(button_callback, pattern="^(niche|turnover|problem|source|restart)_"))
    ...
    app.add_handler(MessageHandler(~filters.Chat(ADMIN_ID) & filters.TEXT & ~filters.COMMAND, handle_thanks))
